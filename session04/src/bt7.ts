// ạo một hàm processInput(input: string | number | boolean) xử lý theo các trường hợp sau:
// Nếu là string:
// Nếu chuỗi có chứa toàn số , hãy chuyển chuỗi đó thành number và in ra bình phương của số đó. 
// Ex : processInput("123") // output: 15129
// Nếu chuỗi chứa chữ, hãy in ra số ký tự chữ cái (bỏ qua số và ký tự đặc biệt  - Có thể áp dụng Regex).
// Ex : processInput("abc123!") // output: 3 ký tự chữ cái
// Nếu là number:
// Kiểm tra số đó có phải số nguyên tố hay không. Nếu có, in ra “Là số nguyên tố”, nếu không thì in ra “Không phải số nguyên tố”.
// Nếu là boolean:
// Nếu true → in ra "Giá trị là true - tiến hành xử lý"
// Nếu false → in ra "Giá trị là false - dừng xử lý"
